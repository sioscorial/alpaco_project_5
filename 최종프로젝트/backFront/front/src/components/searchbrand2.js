...
const BrandItemCircle = styled.div`
  ...
  box-shadow: ${(props) => (props.active ? '0px 8px 16px rgba(0, 0, 0, 0.40)' : 'none')};
...
function Search() {
  ...
  const handleConsonantClick = async (startName, endName) => {
    try {
      ...
      // API 응답에서 'result' 키의 값을 배열로 사용
      const brandsData = response.data.result || [];
      setBrands(brandsData);
    } catch (error) {
      console.error('Error fetching brands:', error);
      setBrands([]); // 오류 발생시 빈 배열 설정
    }
  };
  ...
  const handleSearchChange = (event) => {
    ...
    if (query.trim().length === 0) {
      setIsSearching(false); // 검색어가 없을 경우 검색 상태 비활성화
      handleConsonantClick('가', '나'); // 기본 리스트 다시 로드
    } else {
      fetchBrands(query);
    }
  };
